...
class Basic:
    ...
    def image(path_to_download, theme="cats", number_of_pics=1):
        ...
        browser.find_element_by_xpath(
            "/html/body/div[1]/div/nav[2]/ul/li[2]/form/div/div/div/button/i"
        ).click()
        for num_pics in range(1, number_of_pics + 1):
            try:
                ...
                urllib.request.urlretrieve(
                    src, os.path.join(path_to_download, f"{num_pics}.jpg")
                )
            except:
                print(
                    f"There are no more pics at the first page, you downloaded {num_pics} pics with theme - {theme}"
                )
        browser.close()
        list_of_img_paths = []
        for r, d, f in os.walk(path_to_download):
            for files in f:
                if files.endswith(".jpg"):
                    list_of_img_paths.append(
                        (os.path.join(os.getcwd(), os.path.join(r, files)))
                    )
    ...
    def random_date(start, end=None):
        ...
        return (
            f"{(start + datetime.timedelta(seconds=random_second)).year}-"
