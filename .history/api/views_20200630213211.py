...
from .serializers import (DonationCustomFilter, DonationDetailSerializer,
                          DonationSerializer, PatientCustomFilter,
                          PatientSerializers, UserSerializer)
...
class PublicInfoViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        return Response(
            {'Branches': self.branches,  # returns list of branches
             ...
             'state_of_blood_supply': state_of_blood_supply})  # returns % state_of_blood_supply
    ...
    def retrieve(self, request, branch=None):
        ...
        if branch in self.branches:
            ...
            return Response(status=status.HTTP_404_NOT_FOUND, data={'detail': f'There is no branch: {branch}',
...
class UserViewSet(viewsets.GenericViewSet,
    ...
    search_fields = ['username', 'last_name', 'email', 'profile__position', 'profile__branch']
...
class DonationViewSet(viewsets.ModelViewSet):
    queryset = Donation.objects.select_related('medical_staff', 'medical_staff__profile', 'patient') \
...
class PatientViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        not_able_donation_id = Donation.objects.select_related('patient').filter(
            ...
            Q(accept_donate=True)).values_list('patient__id', flat=True)

        # returns information if donor can donate
        queryset = Patient.objects.select_related('registered_by', 'registered_by__profile') \
            .prefetch_related('donation_set') \
            .annotate(can_donate=models
                      .Case(models.When(id__in=not_able_donation_id, then=False),
                            default=models.Value(True),
                            output_field=models.BooleanField())).order_by('id')
        return queryset
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(status=status.HTTP_423_LOCKED,
                            data={'Information': 'Patient in DB is Protected first '
